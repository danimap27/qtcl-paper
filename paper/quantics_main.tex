...
\begin{enumerate}
  \item \textbf{QTCL framework}: an end-to-end hybrid model with MLP encoder,
  ...
  \item \textbf{Exact QFIM computation}: Fisher weights are computed via adjoint
        differentiation (PennyLane \texttt{lightning.qubit}), providing a
        geometrically principled quantum regularizer.
  \item \textbf{Empirical result}: $+21.3$\,pp average accuracy and $3.3\times$
        less forgetting over the best classical baseline on Split-MNIST.
  \item \textbf{Ablation study}: systematic analysis of EWC strength $\lambda$
...
\noindent\textbf{Variational quantum circuits.}
...
\noindent\textbf{Rehearsal is the critical component.}
...
First, rehearsal is the critical component: QFIM-based EWC alone gives no
benefit over Classical EWC, but its combination with rehearsal produces a
large improvement.
Second, the optimal quantum EWC strength ($\lambda_Q=200$) is lower than for
classical models, which reflects the compact VQC parameter space.
Third, the synergy between QFIM-constrained EWC and rehearsal exceeds the
...
Nat. Rev. Phys. (2024). \href{https://doi.org/10.1038/s42254-024-00784-9}{doi:10.1038/s42254-024-00784-9}
